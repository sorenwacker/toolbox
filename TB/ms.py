...
class example_files:
    def __init__(self):
        self._mzxml = sorted(
            list(glob("/home/swacker/workspace/uofc/data/metabolomics/ms1/mzxml/*"))
        )
        self._parquet = sorted(
            list(glob("/home/swacker/workspace/uofc/data/metabolomics/ms1/parquet/*"))
        )
...
def plot_slize(slize, bins=200, cmin=0, cmax=1000):
    ...
    hist2d(
        ...
        slize["m/z array"],
        bins=bins,
        weights=slize["intensity array"].apply(np.log1p),
        cmin=cmin,
        cmax=cmax,
    )
